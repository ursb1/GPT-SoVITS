...
if ! command -v uv &>/dev/null; then
    echo -e "${INFO}uv not found. Installing uv..."
    pip install uv
    if ! command -v uv &>/dev/null; then
        echo -e "${ERROR}Failed to install uv. Please install it manually: 'pip install uv'"
        exit 1
    fi
...
while [[ $# -gt 0 ]]; do
    ...
    --source)
        ...
        *) echo -e "${ERROR}Error: Invalid Download Source: $2"; exit 1 ;;
        esac; shift 2 ;;
    --device)
        case "$2" in
        ...
        *) echo -e "${ERROR}Error: Invalid Device: $2"; exit 1 ;;
        esac; shift 2 ;;
...
if ! $USE_CUDA && ! $USE_ROCM && ! $USE_CPU; then
    echo -e "${ERROR}Error: Device is REQUIRED"; print_help; exit 1
fi
if ! $USE_HF && ! $USE_HF_MIRROR && ! $USE_MODELSCOPE; then
    echo -e "${ERROR}Error: Download Source is REQUIRED"; print_help; exit 1
fi
...
if [ "$(uname)" != "Darwin" ]; then
    ...
    if [ "$gcc_major_version" -lt 11 ]; then
        ...
        echo -e "${INFO}Detected GCC Version: $gcc_major_version. Installing compatible libstdcxx-ng..."
...
echo -e "${SUCCESS}Base build tools Installed"
...
if [ ! -d "GPT_SoVITS/pretrained_models/sv" ]; then
    ...
    run_wget_parallel "pretrained_models.zip" "$PRETRINED_URL" & pids+=($!)
else
    echo -e "${INFO}  - Skipping Pretrained Models (already exists)"
fi
if [ ! -d "GPT_SoVITS/text/G2PWModel" ]; then
    echo -e "${INFO}  - Queuing G2PWModel..."
    run_wget_parallel "G2PWModel.zip" "$G2PW_URL" & pids+=($!)
else
...
if [ "$DOWNLOAD_UVR5" = "true" ] && ! find -L "tools/uvr5/uvr5_weights" -mindepth 1 ! -name '.gitignore' | grep -q .; then
    ...
    run_wget_parallel "uvr5_weights.zip" "$UVR5_URL" & pids+=($!)
else
    echo -e "${INFO}  - Skipping UVR5 Models (already exists or not requested)"
fi

echo -e "${INFO}  - Queuing NLTK Data..."
run_wget_parallel "nltk_data.zip" "$NLTK_URL" & pids+=($!)
echo -e "${INFO}  - Queuing Open JTalk Dict..."
run_wget_parallel "open_jtalk_dic_utf_8-1.11.tar.gz" "$PYOPENJTALK_URL" & pids+=($!)

echo -e "${INFO}Waiting for all downloads to complete..."
for pid in "${pids[@]}"; do wait "$pid"; done

if [ -f "download.failed" ]; then
    echo -e "${ERROR}One or more parallel downloads failed."; rm -f download.failed; exit 1
fi
echo -e "${SUCCESS}All downloads completed."

echo -e "${INFO}Unpacking general files..."
...
echo -e "${SUCCESS}General files unpacked."

if [ "$USE_CUDA" = true ] && [ "$WORKFLOW" = false ]; then
    if ! command -v nvidia-smi &>/dev/null; then
        echo -e "${WARNING}Nvidia Driver Not Found, Fallback to CPU"; USE_CUDA=false; USE_CPU=true
    fi
fi
if [ "$USE_ROCM" = true ] && [ "$WORKFLOW" = false ]; then
    if [ ! -d "/opt/rocm" ]; then
        echo -e "${WARNING}ROCm Not Found, Fallback to CPU"; USE_ROCM=false; USE_CPU=true
    else
...
echo -e "${INFO}Unpacking language-specific dictionaries..."
if [ -f "open_jtalk_dic_utf_8-1.11.tar.gz" ]; then
    ...
    echo -e "${SUCCESS}Open JTalk Dic unpacked."
fi
